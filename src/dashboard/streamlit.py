...
# Configurações
SIMULAR_LOGIN = True
MEDICOS = ['Dr. Silva - Cardiologia',
           'Dra. Costa - Clínica Geral', 'Dr. Oliveira - Cirurgia']
...
def detectar_anomalias(df):
    try:
        ...
        anomalias_if = iso_forest.fit_predict(
            df[['batimento_cardiaco', 'temperatura', 'pressao_sistolica', 'pressao_diastolica',
                'glicose', 'oxigenio']])
    ...
    try:
        ...
        if real_time:
            ...
            if paciente in st.session_state.dados_acumulados:
                ...
                timestamps = [
                    ultimo_ts + timedelta(seconds=10*(i+1)) for i in range(num_points)]
            else:
                timestamps = [now - timedelta(seconds=10*i)
        ...
        anomaly_indices = np.random.choice(
            num_points, num_anomaly, replace=False)
...
def processar_atividades(df):
    ...
    for i in range(len(df)):
        if df.atividade.iloc[i] == 1 and current_activity is None:
            ...
            duration = (
                current_activity['end'] - current_activity['start']).total_seconds() / 60
            if duration > 0:
                atividades.append({
    ...
    if current_activity is not None:
        ...
        duration = (current_activity['end'] -
                    current_activity['start']).total_seconds() / 60
        atividades.append({
            "Tarefa": "Atividade Física",
            "Início": current_activity['start'],
            "Fim": current_activity['end'],
        ...
        if tempo_real:
            ...
            if paciente not in st.session_state.dados_acumulados:
                ...
                combined_df = pd.concat(
                    [old_df, novo_df]).reset_index(drop=True)
                combined_ativ = pd.concat(
                    [old_ativ, novo_atividades]).reset_index(drop=True)

            combined_df = detectar_anomalias(combined_df)
            st.session_state.dados_acumulados[paciente] = (
                combined_df, combined_ativ)
...
def check_alertas(df):
    ...
    for parametro, atributos in st.session_state.limites.items():
        ...
        if valor < atributos['min']:
            alertas.append(
                f"{parametro.capitalize()} baixa: {valor:.2f} (Mín: {atributos['min']})")
        elif valor > atributos['max']:
            alertas.append(
                f"{parametro.capitalize()} alta: {valor:.2f} (Máx: {atributos['max']})")

    return alertas


with st.sidebar:
    st.markdown('<h1 class="sidebar-title">🏥 PSM 2.0</h1>',
                unsafe_allow_html=True)
        ...
        with st.form("limites_form"):
            ...
            for param, attrs in st.session_state.limites.items():
                ...
                novo_min = col1.number_input(
                    f"Mín {param}", value=attrs['min'])
                novo_max = col2.number_input(
                    f"Máx {param}", value=attrs['max'])
                novos_limites[param] = {
                    'min': novo_min,
                    'max': novo_max,
...
def create_anomaly_chart(df):
    ...
    if not df.empty and 'Anomalia_IF' in df.columns and 'Anomalia_LOF' in df.columns:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers',
            marker=dict(
                color=np.where(df['Anomalia_IF'] == -1,
                               COLOR_PALETTE[2], COLOR_PALETTE[1]),
                size=8,
                line=dict(width=1, color='DarkSlateGrey')
            ),
            text=df.apply(
                lambda row: f"Data: {row['timestamp_str']}<br>Batimento: {row['batimento_cardiaco']}<br>Temperatura: {row['temperatura']:.1f}", axis=1),
            hoverinfo='text',
            showlegend=False
        ), row=1, col=1)
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers',
            marker=dict(
                color=np.where(df['Anomalia_LOF'] == -1,
                               COLOR_PALETTE[2], COLOR_PALETTE[1]),
                size=8,
                line=dict(width=1, color='DarkSlateGrey')
            ),
...
try:
    ...
    if df.empty:
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["Sinais Vitais", "Pressão", "Glicose", "Atividades", "Anomalias"])

        with tab1:
            st.plotly_chart(create_vital_chart(df), use_container_width=True)

        with tab2:
            st.plotly_chart(create_pressure_chart(df),
        ...
        with tab4:
            st.plotly_chart(create_gantt_chart(df_atividades),
                            use_container_width=True)
        ...
        with tab5:
            ...
            if not df_anomalias.empty:
                ...
                st.dataframe(
                    ...
                    column_config={
                        "Data/Hora": "Data/Hora",
                        "Batimento Cardíaco (BPM)": st.column_config.NumberColumn(format="%d"),
                        "Temperatura (°C)": st.column_config.NumberColumn(format="%.1f"),
                        "Pressão Sistólica (mmHg)": st.column_config.NumberColumn(format="%.1f"),
                        "Pressão Diastólica (mmHg)": st.column_config.NumberColumn(format="%.1f"),
            ...
            if not df.empty:
                with st.spinner("Gerando arquivo..."):
                    if formato == "CSV":
                        ...
                        st.download_button(
                            "⬇️ Download CSV", csv, f"dados_{paciente}.csv")
                    else:
                        json = df.to_json(indent=2)
                        st.download_button(
                            "⬇️ Download JSON", json, f"dados_{paciente}.json")
            else:
